...
const velocidadDesvanecimiento = 4; 
let trazosMemoria = []; 
...
// ==========================================
// SUBSISTEMA 1: MEMORIA (Actualizado)
// ==========================================
function dibujarMemoria() {
  for (let i = trazosMemoria.length - 1; i >= 0; i--) {
    ...
    if (trazo.opacidad <= 0) { 
      trazosMemoria.splice(i, 1); 
      continue; 
    }
    
    push(); 
    translate(trazo.x, trazo.y); 
    rotate(trazo.angulo);
    
    // Aplicamos los distintos tonos de verde (claros y oscuros)
    stroke(0, trazo.tonoVerde, 100, trazo.opacidad); 
    noFill(); 
    strokeWeight(1.5);
    
    // Solo dibujamos triángulos
    triangle(0, -trazo.tam, -trazo.tam, trazo.tam, trazo.tam, trazo.tam); 
    pop(); 
  }
  ...
  if (interactuando) {
    let interX = touches.length > 0 ? touches[0].x : mouseX; 
    let interY = touches.length > 0 ? touches[0].y : mouseY;
    let baseTam = min(width, height) * 0.015;
    
    // Generación de la huella de triángulos
    for (let j = 0; j < 3; j++) {
      trazosMemoria.push({ 
        x: interX + random(-35, 35), 
        y: interY + random(-35, 35), 
        tam: random(baseTam * 0.8, baseTam * 1.8), 
        angulo: random(TWO_PI), 
        opacidad: 255,
        tonoVerde: random(80, 255) // Variación de verdes claros y oscuros
      });
    }
    
    // Círculo central reactivo
    // Calculamos un grosor que oscila lentamente entre 2 y 7
    let grosorLento = map(sin(frameCount * 0.05), -1, 1, 2, 7);
    
    stroke(0, 255, 150); 
    fill(15); 
    strokeWeight(grosorLento); 
    circle(interX, interY, min(width, height) * 0.06);
  }
}
